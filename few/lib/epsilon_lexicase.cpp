...
extern "C" void epsilon_lexicase(double * F, int n, int d,
  int num_selections,  int * locs)
{
  // training cases
  ExtMat T (F, n, d);
  // parent locations
  ...
  for (int i = 0; i<num_selections; ++i){
    ...
    while(can_locs.size()>1 && cases.size() > 0){
      ...
      for (int j = 0; j< can_locs.size(); ++j){
        if (j==0 || T(can_locs[j],cases.back())<minfit )
          minfit = T(can_locs[j],cases.back());
        }
      //cout << "minfit: " << minfit << "\n";
      ...
      //  << T(cl,cases.back()) << "\n";
        if (T(cl,cases.back()) <= minfit + epsilon(cases.back())){
          winner.push_back(cl);
          //cout << "<- pass\n";
        }
      }
    ...
    L(i) = *select_randomly(can_locs.begin(),can_locs.end(),gen);
